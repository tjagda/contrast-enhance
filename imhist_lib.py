...
def histeq(im, nbr_bins=256):
    ...
    imhist, bins = numpy.histogram(
        im.flatten(), nbr_bins, (0, 255), density=False)
...
def histmatch(im1, im2, nbr_bins=256):
    ...
    hst1, bins1 = numpy.histogram(
        im1.flatten(), nbr_bins, (0, 255), density=False)
    hst2, bins2 = numpy.histogram(
        im2.flatten(), nbr_bins, (0, 255), density=False)
...
def histeqADAPT(im, radius=20):
    ...
    for i in range(im.shape[0]):
        for j in range(im.shape[1]):
            block = im[max(i-radius, 0):min(i+radius, im.shape[0]),
                       max(j-radius, 0):min(j+radius, im.shape[1])]
